...
1. 先指定「文件名：『中文名称』」，使用『』包裹
2. 使用Markdown代码块返回代码
3. 在文件顶部明确标注了依赖包及版本（示例：# moviepy==1.0.3 即 # 包名==版本号），并在代码中书写pip install 包名的注释
4. 添加适当的错误处理，print报错的时候添加错误符号表情❌
5. 当需要APIkey时，让代码通过.env文件获取，请你提醒用户填入的变量名，变量名使用特殊名，而非API_KEY
6. 添加简单的中文注释
7. 确保代码可以直接运行
8. 如果涉及文件生成，则由代码在文件夹 代码工具库 之下生成目录
9.添加运行日志，日志要求简洁明了，可以考虑附带表情符号
10.代码内指定PYTHON39_PATH = "../venv3.9/Scripts/python.exe"
11.生成的代码，结果呈现需要时间的，自行决定使用os.system("pause") 还是time.sleep(20)，自行判断是否需要循环
12.当代码涉及文件选择时，优先使用tkinter打开文件选择器，使用DPI感知windll.shcore.SetProcessDpiAwareness(1)
13.你仅适合生成简单的python代码，当用户提及时，请告诉他"""
        }]#提示词
